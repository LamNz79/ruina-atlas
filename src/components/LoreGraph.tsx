...
import type { EdgeType, GraphEdge, Sinner } from '../types';
import { FilterPanel } from './FilterPanel';
import { GraphSettings } from './GraphSettings';
import GraphLegend from './GraphLegend';

import type { FilterState, GraphNode, PhysicsSettings } from './LoreGraphConstants';
import {
  ALL_EDGE_TYPES,
  DEFAULTS,
  EDGE_COLORS,
...
export interface LoreGraphProps {
  sinners: Sinner[];
  edges: GraphEdge[];
  ...
  const renderNodeContent = useCallback((el: d3.Selection<SVGGElement, GraphNode, any, any>) => {
    el.each(function (d) {
      ...
      if (d.nodeType === 'literary-source') {
        ...
        const sourceData = literarySources.find((ls: any) => ls.id === sourceId);
